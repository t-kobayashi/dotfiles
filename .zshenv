path=(
    /usr/local/go/bin(N-/)
    /usr/local/opt/mysql-client/bin(N-/)
    /usr/local/opt/gettext/bin(N-/)
    /usr/local/android/cmdline-tools/latest/bin(N-/)
    /usr/local/android/build-tools/36.0.0(N-/)
    /usr/local/android/platform-tools(N-/)
    /usr/local/android/emulator(N-/)
    /usr/local/android/tools(N-/)
    /usr/local/android/tools/bin(N-/)
    /usr/local/android/build-tools/33.0.2(N-/)
    /opt/homebrew/bin(N-/)
    /opt/homebrew/opt/mysql-client/bin(N-/)
    /opt/homebrew/opt/mysql-client@8.0/bin(N-/)
    /opt/homebrew/opt/mysql-client@8.4/bin(N-/)
    /opt/homebrew/opt/openjdk@17/bin(N-/)
    /opt/homebrew/share/android-commandlinetools/platform-tools(N-/)
    /opt/homebrew/share/android-commandlinetools/cmdline-tools/latest/bin(N-/)
    /opt/homebrew/share/android-commandlinetools/build-tools/34.0.0(N-/)
    "/Applications/Visual Studio Code.app/Contents/Resources/app/bin"(N-/)
    ~/bin(N-/)
    ~/go/bin(N-/)
    ~/flutter/bin(N-/)
    ~/node_modules/.bin(N-/)
    ~/.local/bin(N-/)
    ~/.local/share/gem/ruby/3.0.0/bin(N-/)
    ~/.pub-cache/bin(N-/)
    ~/.deno/bin(N-/)
    ~/.script(N-/)
    ~/.tfenv/bin(N-/)
    ~/ghq/github.com/flutter/flutter/bin(N-/)
    $path
)
